...
bot.on('ready', () => {
    ...
    bot.user.setActivity("habbocity.me");
    return bot.channels.get("546036107163336744").send("Démarrage du bot réussi")
  });
...
bot.on("guildMemberAdd", member => {
    ...
    .setDescription("Nouveau membre sur eHabbo Community (" + member.guild.memberCount + ")")
    ...
    // Commande animation
        bot.on("message", async (message) => {
            ...
            if(cmd === '!anim' && cityArray.length > 1){
                ...
                .setThumbnail("https://ehabbo.eu/ppbot.png")
...
bot.on("message", async (message) => {
    ...
    if(cmd === '!citystatus'){
        let newsembed = new Discord.RichEmbed()
        .setTitle("eHabbo")
		.setDescription("Status du serveur eHabbo")
        .setColor("#feb063") 
